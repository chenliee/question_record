...
# 2024.12.20
1. android配置applink 實現網頁跳轉app，當時瀏覽器輸入url依舊重載url 無法跳轉 解決方法 在url放個靜態文件，打開默認加載該url即可實現(暫時解決方法)
...
4. pop install報錯：[!] CocoaPods could not find compatible versions for pod "TXIMSDK_Plus_iOS":
  	...
  	In Podfile:
    	tencent_cloud_chat_sdk (from `.symlinks/plugins/tencent_cloud_chat_sdk/ios`) was resolved to 7.0.0, which depends on
